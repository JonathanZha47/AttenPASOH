"""
    The same as AttenPredict_DynaWeight_PI.py

    add the atten-weight output
"""
import torch
...
class DualPINN(nn.Module):
    ...
    def forward(self, xt, x_spatio):
        ...
        G, weight_atten = self.spatiodynamic_G(x_spatio)
        G = G.unsqueeze(-1)
        g1 = G[0]
        g2 = G[1]
        g3 = G[2]
        f = u_t - F
        return u, f, g1, g2, g3, weight_atten
    ...
    def train_one_epoch(self, epoch, dataloader, spatiodataloader):
        ...
        for (data, spatiodata) in zip(dataloader, spatiodataloader):
            ...
            u, f, g1, g2, g3, weight_atten = self.forward(x, x_spatio)
        ...
        return loss1_meter.avg, loss2_meter.avg, loss3_meter.avg, loss4_meter.avg, alpha_meter.avg, beta_meter.avg, theta_meter.avg, weight_atten
    ...
    def Train(self, trainloader, testloader=None, validloader=None, spatiotrainloader=None, spatiovalidloader=None,
        ...
        for e in range(1, self.args.epochs + 1):
            early_stop += 1
            loss1, loss2, loss3, loss4, alpha, beta, theta, weight = self.train_one_epoch(e, trainloader,spatiotrainloader)
            current_lr = self.scheduler.step()
            current_lr2 = self.scheduler2.step()
            info = '[Train] epoch:{}, lr:{:.6f}, alpha:{:.4f} , beta:{:.4f}, gamma:{:.4f},theta:{:.4f} ' \
                   'total loss:{:.6f}'.format(e, current_lr, alpha, beta, self.gamma, theta,
            ...
            if valid_mse < min_valid_mse and testloader is not None:
                ...
                if self.args.save_folder is not None:
                    ...
                    weight_cpu = [w.detach().cpu().numpy() for w in weight]  # 如果 weight 是一个列表
                    np.save(os.path.join(self.args.save_folder, 'atten_weight.npy'), weight_cpu)
                ##################################################################################
