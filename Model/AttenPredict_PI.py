"""
    In this file, AttenPredict_PI is using seq2seq attention mechanism to use series data to generate multi-step series for constructing the soft-constraint
    physics loss and also it includes the original PINN PDE loss and data loss.
    The only difference is that it did not use the dynamic weight assignment mechanism.

    part of ablation study for comparing the use of the dynamic weight assignment mechanism.

    validate the use of the dynamic weight assignment mechanism.
...
from Model.Modules.AttenSpatio import AttenSpatio
    
class DualPINN(nn.Module):
    def __init__(self,args):
        super(DualPINN, self).__init__()
        self.args = args
        ...
        self.spatiodynamic_G = AttenSpatio(self.args,input_dim=17, d_model=64, 
                                                    n_heads=8, n_layers=4, d_ff=256, 
                                                    d_k=8, d_v=8, dropout=0.1, seq_length=3).to(device)
        ...
        self.optimizer3 = torch.optim.Adam(self.spatiodynamic_G.parameters(), lr=args.F_warmup_lr)
        ...
        self.optimizer_weights = torch.optim.Adam([torch.tensor(self.alpha,requires_grad=True),
                                                   torch.tensor(self.beta,requires_grad=True),
                                                   torch.tensor(self.gamma,requires_grad=True),
                                                   torch.tensor(self.theta,requires_grad=True)],lr=0.0005)
    ...
    def forward(self,xt,x_spatio):
        ...
        G = self.spatiodynamic_G(x_spatio).unsqueeze(-1)
        g1 = G[0]
        g2 = G[1]
        g3 = G[2]
        f = u_t - F
    ...
    def train_one_epoch(self,epoch,dataloader,spatiodataloader):
        self.train()
        loss1_meter = AverageMeter()
        loss2_meter = AverageMeter()
        loss3_meter = AverageMeter()
        for (data,spatiodata) in zip(dataloader,spatiodataloader):
            x,y = data
            x_spatio,y_spatio = spatiodata
            x, y = x.to(device), y.to(device)
            x_spatio, y_spatio = x_spatio.to(device), y_spatio.to(device)

            u,f,g1,g2,g3 = self.forward(x,x_spatio)
            ...
            # Physics loss
            loss3 = (torch.relu(g2-g1) + torch.relu(g3-g2)).mean()

            # spatio loss
            loss4 = self.loss_func((g1+g2+g3).mean(),y_spatio.mean())
            # total loss
            loss = self.alpha*loss1 + self.beta*loss2 + self.gamma*loss3 + loss4
    ...
    def Train(self,trainloader,testloader=None,validloader=None,spatiotrainloader=None, spatiovalidloader=None, spatiotestloader=None):
        min_valid_mse = 10
        valid_mse = 10
        early_stop = 0
        mae = 10
        for e in range(1,self.args.epochs+1):
            early_stop += 1
            loss1,loss2,loss3 = self.train_one_epoch(e,trainloader,spatiotrainloader)
            current_lr = self.scheduler.step()
            current_lr2 = self.scheduler2.step()
            info = '[Train] epoch:{}, lr:{:.6f}, ' \
                   'total loss:{:.6f}'.format(e,current_lr,self.alpha*loss1+self.beta*loss2+self.gamma*loss3)
