...
class NewPhysicsPINN(nn.Module):
    def __init__(self, args):
        super(NewPhysicsPINN, self).__init__()
        self.args = args
    ...
    def forward(self, xt, x_spatio):
        ...
        G= self.spatiodynamic_G(x_spatio)
        G = G.unsqueeze(-1)
        f = u_t - F
        return u, f, G
    
    def physics_loss(self, G: torch.Tensor, y_spatio: torch.Tensor) -> torch.Tensor:
        """
        Compute the Physics loss based on the difference between the trends (sign of differences) 
        between predicted sequence `G` and true sequence `y_spatio`.

        :param G: Predicted sequence, shape [batch_size, seq_length, 1, 1]
        :param y_spatio: True sequence, shape [batch_size, seq_length]
        :return: The computed physics loss.
        """
        # Squeeze G to match the shape of y_spatio
        G = G.squeeze(-1).squeeze(-1)  # shape: [batch_size, seq_length]

        # Compute consecutive differences for G (predicted) and y_spatio (true)
        g_diff = G[:, 1:] - G[:, :-1]  # shape: [batch_size, seq_length - 1]
        y_diff = y_spatio[:, 1:] - y_spatio[:, :-1]  # shape: [batch_size, seq_length - 1]
        ...
        # Compute the absolute difference between the trends of G and y_spatio
    ...
    def train_one_epoch(self, epoch, dataloader, spatiodataloader):
        ...
        for (data, spatiodata) in zip(dataloader, spatiodataloader):
            ...
            # Physics loss
            loss3 = self.physics_loss(G, y_spatio)

            # spatio loss
            loss4 = self.loss_func(G.squeeze(-1).squeeze(-1).mean(), y_spatio.mean())
            # total loss
    ...
    def Train(self, trainloader, testloader=None, validloader=None, spatiotrainloader=None, spatiovalidloader=None,
              spatiotestloader=None):
        min_valid_mse = 10
        valid_mse = 10
        early_stop = 0
        mae = 10
        for e in range(1, self.args.epochs + 1):
            early_stop += 1
            loss1, loss2, loss3, loss4, alpha, beta, gamma, theta = self.train_one_epoch(e, trainloader,spatiotrainloader)
