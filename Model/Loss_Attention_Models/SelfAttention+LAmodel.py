...
class FeatureAttention(nn.Module):
    #TODO: set up the initial dimention parameters of the model
    def __init__(self, query_dim, key_dim, value_dim, attention_dim):
        ...
        self.value_layer = nn.Linear(value_dim, attention_dim)
        self.softmax = nn.Softmax(dim=1)
    
    def forward(self, query, key, value):
        # Compute query, key, and value projections
        queries = self.query_layer(query)  # (batch_size, attention_dim)
        keys = self.key_layer(key)         # (batch_size, attention_dim)
        values = self.value_layer(value)   # (batch_size, attention_dim)
        
        # Compute attention scores (similarity between queries and keys)
        attention_scores = torch.bmm(queries.unsqueeze(1), keys.unsqueeze(2)).squeeze(-1)  # (batch_size, 1)
        
        # Normalize the scores to get the attention weights
        attention_weights = self.softmax(attention_scores)  # (batch_size, 1)
        
        # Compute the context vector as a weighted sum of the values
        context_vector = attention_weights.unsqueeze(2) * values  # (batch_size, attention_dim)
        
        # Summing over the sequence length dimension
        context_vector = torch.sum(context_vector, dim=1)  # (batch_size, attention_dim)
        
        return context_vector, attention_weights

class DualInputAttentionModel(nn.Module):
    def __init__(self, sim_input_dim, true_input_dim, output_dim):
        ...
        # Attention mechanism with classic Q-K-V structure
        self.attention = FeatureAttention(32, 32, 32, 32)  # Assuming Q, K, V are all 32-dimensional
    ...
    def forward(self, sim_data, true_data):
        ...
        sim_features = self.sim_fc(sim_data)  # (batch_size, 32)
        
        # Process true data
        true_features = self.true_fc(true_data)  # (batch_size, 32)
        
        # Apply attention (using simulated data as query, true data as key and value)
        # TODO: check if the attention mechanism is correct, reconsider what to be query, key and value
        context_vector, attn_weights = self.attention(sim_features, true_features, true_features)
    ...
    ##这里的attn_weights 可以后面为我们去可视化sim_data 和 true_data的attention weights，以便于我们更好的理解模型的学习过程，以及模型的学习到了什么特征（特征的重要性）
...
class Solution_u(nn.Module):
    def __init__(self):
        ...
        self.transformer = DualInputAttentionModel(17,17,1)
        self._init_()
...
class LAPINN(nn.Module):
    ...
    def forward(self, xt):
        ...
        x = xt[:, 0:-1]
        t = xt[:, -1:]
        u, attn_weight = self.solution_u(torch.cat(x,t), dim=1)
