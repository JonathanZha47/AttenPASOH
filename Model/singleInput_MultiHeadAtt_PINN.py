"""
    In this file, this is a complete model which used the attention mechanism as the solution_u,
    and the 
"""
...
class AttenPINN(nn.Module):
    def __init__(self,args):
        super(AttenPINN, self).__init__()
        self.args = args
        ...
        self.solution_u = MultiHeadAtten(self.args,input_dim=17, d_model=64, 
        ...
        self.optimizer_weights = torch.optim.Adam([torch.tensor(self.alpha,requires_grad=True),
                                                   torch.tensor(self.beta,requires_grad=True)],lr=0.0003)
