...
class SplineActivation(nn.Module):
    def __init__(self, in_features, grid_size=60, spline_order=3, scale_noise=0.1, scale_spline=1.0, grid_range=[-1, 1]):
    ...
    def reset_parameters(self):
        with torch.no_grad():
            noise = (
                (torch.rand(self.grid_size + 1, self.in_features) - 0.5)
                * self.scale_spline
                / self.grid_size
            )
    ...
    def curve2coeff(self, x: torch.Tensor, y: torch.Tensor):
        ...
        result = solution.permute(1, 0)

        assert result.size() == (self.in_features, self.grid_size + self.spline_order)
        return result.contiguous()

    def forward(self, x: torch.Tensor):
        bases = self.b_splines(x)
        output = torch.einsum('bij,jk->bik', bases, self.spline_weight.T)
    ...
    def __init__(self,input_dim=17,output_dim=1,layers_num=4,hidden_dim=50,droupout=0.2):
        ...
        for i in range(layers_num):
            if i == 0:
                ...
                self.layers.append(SplineActivation(hidden_dim,grid_size=self.hidden_dim))
            elif i == layers_num-1:
...
class Predictor(nn.Module):
    def __init__(self,input_dim=40):
        ...
        self.net = nn.Sequential(
            ...
            SplineActivation(32,grid_size=32),  # Replace with SplineActivation
            nn.Linear(32,1)
        )
        self.input_dim = input_dim
    def forward(self,x):
...
class DynamicAFPINN(nn.Module):
    def __init__(self,args):
        super(DynamicAFPINN, self).__init__()
        self.args = args
