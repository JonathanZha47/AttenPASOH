...
from Model.Compare_Models.Autoformer.Reformer import Reformer
import os
...
class ReformerNet(nn.Module):
    def __init__(self,args):
        super(ReformerNet, self).__init__()
        self.args = args
        ...
        self.Reformer = Reformer(args).to(device)
        self.loss_func = nn.MSELoss()
        self.relu = nn.ReLU()
        self.optimizer = torch.optim.Adam(self.Reformer.parameters(),lr=0.001)
    ...
    def load_model(self, model_path):
        ...
        self.Reformer.load_state_dict(checkpoint['Reformer'])
        for param in self.Reformer.parameters():
            param.requires_grad = True
    
    def predict(self,xt):
        return self.Reformer(xt)
    ...
    def Train(self,trainloader,testloader=None,validloader=None):
        ...
        for e in range(1,self.args.epochs+1):
            ...
            if valid_mse < min_valid_mse and testloader is not None:
                ...
                ############################### save ############################################
                self.best_model = {'Reformer':self.Reformer.state_dict()}
