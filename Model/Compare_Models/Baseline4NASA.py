"""
    In this file, this mainly contains the implementation of the PINN4SOH on NASA dataset.
    Since the processed NASA data is in dimention of 49, so we changed the input dimension and output dimension of each modules in the PINN4SOH.
    - in the architecture of the solution_u, we changed the below parameters:
        - u_input_dim, u_output_dim, u_layers_num, u_hidden_dim(which you should change them in command line when calling the main function)
    - in the architecture of the dynamical_F, we changed the below parameters:
        - F_layers_num, F_hidden_dim(which you should change them in command line when calling the main function)
        - the input dimension of the dynamical_F is u_input_dim * 2 + 1
"""
...
class Solution_u(nn.Module):
    def __init__(self):
        ...
        self.encoder = MLP(input_dim=args.u_input_dim,output_dim=args.u_output_dim,layers_num=args.u_layers_num,hidden_dim=args.u_hidden_dim,droupout=0.2)
...
class PINN(nn.Module):
    def __init__(self,args):
        ...
        self.dynamical_F = MLP(input_dim=args.u_input_dim * 2 + 1,output_dim=1,
    ...
    def train_one_epoch(self,epoch,dataloader):
        ...
        for iter,(x,y) in enumerate(dataloader):
            ...
            if (iter+1) % 50 == 0:
                print("[epoch:{} iter:{}] data loss:{:.6f}, PDE loss:{:.6f}, physics loss:{:.6f}".format(epoch,iter+1,loss1,loss2))
