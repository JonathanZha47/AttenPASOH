...
from Model.AttenPredict_DynaWeight_PI import DualPINN
import argparse
import os
import sys
...
def main():
    ...
    for batch_size in batch_size_options:
        ...
        for e in range(10):  # Run the experiments
            # Define the save folder for the results based on the batch size and experiment number
            save_folder = f'results of reviewer/XJTU-AttenPASOH(Batch Size)/{batch_size}/Experiment{e + 1}'
            if not os.path.exists(save_folder):
                os.makedirs(save_folder)
            log_dir = 'logging.txt'
            
            # Set the save folder and log directory for the current experiment
            ...
            # Train the model with the current batch size and experiment setup
            pinn.Train(trainloader=dataloader['train'], validloader=dataloader['valid'], testloader=dataloader['test'], spatiotrainloader=dataloader['spatio_train'], spatiovalidloader=dataloader['spatio_valid'], spatiotestloader=dataloader['spatio_test'])

    print("Batch size experiment finished")
"""
