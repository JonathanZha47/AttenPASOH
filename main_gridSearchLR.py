...
from Model.Loss_Attention_Models.LAPINN4SOH import LAPINN
import argparse
import os
...
def main():
    ...
    solution_u_lr_options = [0.002, 0.001, 0.0005]
    F_lr_options = [0.002, 0.001, 0.0005]
    lan_lr_options = [0.001, 0.0005, 0.0002]

    # Iterate over all combinations of learning rates
    for solution_u_lr, F_lr, lan_lr in itertools.product(solution_u_lr_options, F_lr_options, lan_lr_options):
        setattr(args, 'warmup_lr', solution_u_lr)
        setattr(args, 'lr_F', F_lr)
        setattr(args, 'lr_lan', lan_lr)
        
        for e in range(10):
            save_folder = f'results of reviewer/XJTU(LAPINN) results6-L1-L2/{batch}/Experiment{e + 1}_LR_u{solution_u_lr}_F{F_lr}_lan{lan_lr}'
...
def get_args():
    ...
    # scheduler related
    parser.add_argument('--epochs', type=int, default=600, help='epoch')
    parser.add_argument('--early_stop', type=int, default=60, help='early stop')
    ...
    parser.add_argument('--lr_lan', type=float, default=1e-4, help='learning rate of LAN')
