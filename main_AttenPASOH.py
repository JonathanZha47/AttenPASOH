from dataloader.dualloader2 import *
from Model.seq2seq_modifiedphysics_dynaWeight import DualPINN
import argparse
import os
import sys
import torch
...
def load_NASA_data(args, small_sample=None):
    ...
    dataloader = {'spatio_train': loader['train_spatio'], 'spatio_valid': loader['valid_spatio'],
                  'spatio_test': loader['test_spatio'],
...
def load_XJTU_data(args, small_sample=None):
    ...
    loader = data.read_all(specific_path_list=data_list,noise_mean=args.noise_mean,noise_std=args.noise_std)
...
def main():
    ...
    for i in range(1,6):
        ...
        for e in range(10):
            save_folder = 'results of reviewer/XJTU-attenPASOH(2C+32+0.25equalweight+noisy) results/' + str(i) + '-' + str(
            ...
            pinn.Train(trainloader=dataloader['train'], validloader=dataloader['valid'], testloader=dataloader['test'],
                       spatiotrainloader=dataloader['spatio_train'], spatiovalidloader=dataloader['spatio_valid'],
                       spatiotestloader=dataloader['spatio_test'])
