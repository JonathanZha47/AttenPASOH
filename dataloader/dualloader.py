...
class DF():
    ...
    def load_one_battery(self,datasource,path,noise_mean,noise_std,nominal_capacity=None):
        '''
        ...
        if datasource == 'NASA':
            ...
            if df.columns[-1] == 'capacity':
                ...
                raise ValueError("The column at index 15 is not 'capacity'. Please check the input data format.")
            return (x_noisy, y)
    ...
    def load_all_battery(self,path_list,nominal_capacity,noise_mean,noise_std):
        '''
        ...
        for path in path_list:
            (x,y) = self.load_one_battery(self.args.datasource,path, nominal_capacity,noise_mean,noise_std)
            # print(path)
...
class XJTUdata(DF):
    ...
    def read_one_batch(self,batch='2C'):
        ...
        return self.load_all_battery(path_list=file_list,nominal_capacity=self.nominal_capacity,noise_mean=self.noise_mean,noise_std=self.noise_std)

    def read_all(self,noise_mean,noise_std,specific_path_list=None):
        '''
        ...
        if specific_path_list is None:
            ...
            return self.load_all_battery(path_list=specific_path_list,nominal_capacity=self.nominal_capacity,noise_mean=self.noise_mean,noise_std=self.noise_std)
