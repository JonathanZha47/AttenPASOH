...
class DF():
    ...
    def load_one_battery(self,datasource,path,nominal_capacity=None):
        ...
        if datasource == 'NASA':
            ...
            if df.columns[-1] == 'capacity':
                ...
                raise ValueError("The column at index last is not 'capacity'. Please check the input data format.")
            return (x, y)
    ...
    def load_all_battery(self,path_list,nominal_capacity):
        ...
        for path in path_list:
            (x,y) = self.load_one_battery(path, nominal_capacity,self.args.datasource)
            # print(path)
